...
function initMap() {
    const defaultLocation = { lat: 27.496, lng: -109.940 }; // Coordenadas de Obregón
...
function buildSlider(images) {
    ...
    images.forEach((imgSrc, index) => {
        ...
        sliderHTML += `<img src="${fullSrc}" class="slider-image ${index === 0 ? 'active' : ''}" alt="Producto imagen ${index + 1}">`;
...
function addMarkersToMap(tiendas) {
    ...
    tiendas.forEach(tienda => {
        ...
        li.innerHTML = `<b>${tienda.nombre}</b>: ${tienda.direccion || 'Dirección no disponible'} (Stock: ${tienda.cantidadEnTienda})`;
    ...
    if (!bounds.isEmpty()) {
        ...
        document.getElementById('map').innerHTML = '<p style="text-align:center; padding-top: 50px;">Ubicaciones no disponibles en el mapa.</p>';
    }
}
...
function displayProductDetails(data) {
    ...
    infoContainer.innerHTML = `
        ...
        <a href="https://wa.me/526441901249?text=Hola,%20me%20interesa%20el%20producto:%20${encodeURIComponent(data.nombre)}" target="_blank" class="order-button">Ordenar Aquí (WhatsApp)</a>
    `;
